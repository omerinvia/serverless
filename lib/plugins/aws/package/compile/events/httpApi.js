...
class HttpApiEvents {
  ...
  getProviderTags() {
    if (
      ...
      return Object.assign({}, this.serverless.service.provider.tags);
    }
    return {};
  }
  getApiIdConfig() {
  ...
  compileApi() {
    ...
    properties.Tags = this.getProviderTags();
  ...
  compileCustomDomains() {
    for (const [domainName, domainDefinition] of Object.entries(this.config.customDomains)) {
      this.cfTemplate.Resources[this.provider.naming.getHttpApiDomainNameLogicalId(domainName)] = {
        Type: 'AWS::ApiGatewayV2::DomainName',
        Properties: {
          DomainName: domainDefinition.path,
          DomainNameConfigurations: [{ CertificateArn: domainDefinition.sslCertificateArn }],
          Tags: this.getProviderTags(),
        },
      };
      this.cfTemplate.Resources[this.provider.naming.getHttpApiApiMappingLogicalId(domainName)] = {
        Type: 'AWS::ApiGatewayV2::ApiMapping',
        DependsOn: [
          this.provider.naming.getHttpApiDomainNameLogicalId(domainName),
          this.provider.naming.getHttpApiLogicalId(),
          this.provider.naming.getHttpApiStageLogicalId(),
        ],
        Properties: {
          ApiId: { Ref: this.provider.naming.getHttpApiLogicalId() },
          Stage: { Ref: this.provider.naming.getHttpApiStageLogicalId() },
          DomainName: { Ref: this.provider.naming.getHttpApiDomainNameLogicalId(domainName) },
          Tags: this.getProviderTags(),
        },
      };
      this.cfTemplate.Resources[this.provider.naming.getHttpApiRecordSetLogicalId(domainName)] = {
        Type: 'AWS::Route53::RecordSet',
        DependsOn: [this.provider.naming.getHttpApiDomainNameLogicalId(domainName)],
        Properties: {
          HostedZoneName: domainDefinition.hostedZoneName,
          Name: domainDefinition.path,
          Type: 'A',
          AliasTarget: {
            DNSName: {
              'Fn::GetAtt': [
      ...
      this.cfTemplate.Outputs[this.provider.naming.getHttpApiDomainNameIdLogicalId(domainName)] = {
        Description: 'HttpApi custom api domain id',
        Value: { Ref: this.provider.naming.getHttpApiDomainNameLogicalId(domainName) },
      };
    }
  }
  compileAuthorizers() {
  ...
  memoizeeMethods({
    resolveConfiguration: d(function () {
      ...
      this.config = {
        ...
        customDomains: userConfig.customDomains || [],
      };
      let cors = null;
